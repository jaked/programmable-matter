  ...
  _recordChange = (record: Record, overwrite: boolean = false) => {
    ...
    if (overwrite) {
      ...
      if (last && timestamp - last.timestamp < HISTORY_TIME_GAP) {
        ...
        const previousLine = this._getLines(last.value, last.selectionStart).pop();
        const previous = previousLine ? previousLine.match(re) : null;

        // Get the current line
        const currentLine = this._getLines(record.value, record.selectionStart).pop();
        const current = currentLine ? currentLine.match(re) : null;
  ...
  _handleKeyDown = e => {
    ...
    if (e.keyCode === KEYCODE_TAB && !ignoreTabKey && this.state.capture) {
      ...
      if (selectionStart === selectionEnd) {
        ...
        const matches = line ? line.match(/^\s+/) : null;
...
const styles = { container, textarea, highlight, editor };
